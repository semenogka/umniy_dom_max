...
class House(Base):
    ...
    messages = relationship(
        "HouseMessage", cascade="all, delete-orphan", order_by="HouseMessage.created_at"
    )
...
class MessageAttachment(Base):
    ...
    id = Column(Integer, primary_key=True)
    appeal_message_id = Column(
        ForeignKey("appeal_messages.id", ondelete="CASCADE"), nullable=True, index=True
    )
    house_message_id = Column(
        ForeignKey("house_messages.id", ondelete="CASCADE"), nullable=True, index=True
    )
    url = Column(String)
    ord = Column(Integer)
    __table_args__ = (
        CheckConstraint(
            "(appeal_message_id IS NOT NULL) != (house_message_id IS NOT NULL)"
        ),
    )
...
class User(Base):
    ...
    appeals = relationship(
        "Appeal", back_populates="author", cascade="all, delete-orphan"
    )
